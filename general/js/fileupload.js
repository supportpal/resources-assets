...
function FileUpload(parameters)
{
    ...
    var handleFailedUpload = function (data, message, $context) {
        ...
        if (! $('.sp-alert-error').length) {
            if ($context.parents('.sp-form-container').length && false) {
                $context.parents('.sp-form-container').before('<div class="sp-alert sp-alert-error sp-hidden"></div>');
            } else {
                // In case it's not in a form-container
                $('label.file-input.button').before('<div class="sp-alert sp-alert-error sp-hidden" style="margin: 0.75rem 0"></div>');
            }
        }

        // Show error message.
        var $box = $('.sp-alert-error.attachment').length ? $('.sp-alert-error.attachment') : $('.sp-alert-error'),
            $container = $box,
        ...
        $('html, body, #content').animate({ scrollTop: $box.show().delay(10000).fadeOut().position().top - 24 }, 500);
    };
