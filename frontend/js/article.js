$(document).ready(function () {
  ...
  $('.sp-comment-ordering').on('change', function () {
    ...
    // Make call
    $.get(typeof commentRoute !== 'undefined' ? commentRoute : laroute.route('selfservice.comment'), {
      "articleId": articleId,
      "typeId": typeId,
      "order": $(this).val()
    }, function (response) {
    ...
    $.post(laroute.route('selfservice.article.rating', {
      ...
      if (response.status == 'success') {
        ...
        $.get(laroute.route("selfservice.article.feedback", {
          ...
          // No feedback form
          if (response.data === null) {
            $this.parents('.sp-rate-article-container').text(Lang.get('selfservice.thank_you_for_feedback'));
            return Swal.fire({
              title: null,
