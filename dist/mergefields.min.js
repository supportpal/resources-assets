!function (t, e) {
  ...
  function e(t) {
    ...
    this.callback = function (t) {
      !function (t) {
        /\{\{\s*operator\.reply_template(\|raw)?\s*}}/.test(t) ? e.find(".twig-sig-warning").length || e.append($("<div>", {
          class: "sp-alert sp-alert-warning sp:mt-3 sp:mb-0 twig-sig-warning",
          text: Lang.get("core.twig_operator_reply_template")
        })) : e.find(".twig-sig-warning").remove();
      }(t);
    }, this.showPreview = function () {
      var r = function (t) {
        n.find("button.mergefields-switch-view:visible").prop("disabled", !1).trigger("click"), Swal.fire(Lang.get("messages.error"), t || Lang.get("messages.general_error"), "error");
      };
      n.find("button.mergefields-switch-view").prop("disabled", !0), a.html("").css(s.position()).css("width", s.outerWidth(!0)).css("height", s.outerHeight(!0)).addClass("loadinggif").show();
      var d = e.parents("form").find(':input[name="brand_id"]').length ? e.parents("form").find(':input[name="brand_id"]').val() : e.parents(".sp-form-row").data("brand"),
        i = e.parents("form").find(':input[name="ticket_id"]').length ? e.parents("form").find(':input[name="ticket_id"]').val() : null,
        u = e.parents(".sp-form-container").find(':input[name$="[language]"]').length ? e.parents(".sp-form-container").find(':input[name$="[language]"]').val() : Lang.locale();
      $.post(laroute.route("core.operator.emailtemplate.preview"), {
        template: o(),
        locale: u,
        template_id: e.parents("form").data("templateId"),
        brand_id: d,
        ticket_id: i,
        is_email: t.syntaxEmailTemplate ? 1 : 0
      }).done(function (t) {
        void 0 !== t.data ? a.html(t.data) : r();
      }).fail(function (t, e, n) {
        try {
          var a = JSON.parse(t.responseText);
          r(void 0 !== a.message ? a.message : n);
        } catch (t) {
          r(n);
        }
      }).always(function () {
        n.find("button.mergefields-switch-view").prop("disabled", !1), a.removeClass("loadinggif");
      });
      ...
      s = t, e = $('<div class="sp-editor-container"></div>').appendTo(s.parent()), a = $('<div class="sp-editor-preview sp-editor-content"></div>').hide(), e.append(a);
      var o,
        i = s.parent().find(".sp-editor-buttons-toolbar").first();
      i.length ? (n = i, o = $(d()).children("button"), n.append(o)) : (o = (n = $(d())).find("button"), e.append(n)), o.on("click", function (t) {
        t.preventDefault(), $(this).hasClass("visual-preview") ? r.showPreview() : r.showEditor(), n.find(".mergefields-switch-view").toggleClass("sp:hidden");
      });
    };
  }
  e.translations = {
  ...
  }, e.appendTo = function (t, n, a, s) {
    return t.find(".sp-merge-fields").append(n ? e.ticketTemplate(a) : "").append(e.userAndSystemTemplate(s)).find("button").on("click", function () {
