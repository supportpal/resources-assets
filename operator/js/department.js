jQuery(function ($) {
  ...
  $('select[name="brand[]"]').selectize({
    ...
    onChange: function (values) {
      ...
      selectize.load(function (callback) {
        xhr && xhr.abort();
        xhr = $.ajax({
          url: laroute.route('ticket.operator.department.parents', {
            'department': $('#departmentForm').data('id'),
            'brands': values
          }),
  ...
  $('#emailAccounts').on('change', '.email-support', function () {
    ...
    if ($(this).val() == '0') {
      $email.find('.email-download, .email-piping, .remote-piping').hide().find(':input').prop('disabled', true);
    } else if ($(this).val() == '1') {
      $email.find('.email-download').show().find(':input').prop('disabled', false);
      $email.find('.email-piping, .remote-piping').hide().find(':input').prop('disabled', true);
      ...
      $email.find('.email-download, .remote-piping').hide().find(':input').prop('disabled', true);
      $email.find('.email-piping').show().find(':input').prop('disabled', false);
    } else if ($(this).val() == '3') {
      $email.find('.email-download, .email-piping').hide().find(':input').prop('disabled', true);
      ;
  ...
  .on('click', '.get-access-token, .reset-access-token', function () {
    ...
    $email.oauth(options).done(function () {
  ...
  function autoConfigure($email) {
    ...
    $.get("https://autoconfig.thunderbird.net/v1.1/" + domain).then(function (data, textStatus, jqXHR) {
      if (jqXHR.status === 404) {
        throw new Error();
      }
      var $xml = $(data),
  ...
  function validateEmail(data, item) {
    $.post(laroute.route('ticket.operator.department.validate'), data).then(function (response, textStatus, jqXHR) {
