;(function (global) {
    ...
    var createList = function (tickets) {
        var ticketList = $('<ul />')
            .addClass('sp-list-none sp-p-0 sp-m-0 sp-border sp-border-secondary sp-rounded sp-text-left sp-overflow-y-auto')
        ...
        $.each(tickets, function (index, value) {
            ...
            $li.append(
                $('<a />').text('#' + value.number + ' - ' + value.subject)
                        .prop('href', value.operator_url)
                        .prop('target', '_blank')
            )
                .append('<br />')
                .append($('<img />').prop('src', value.user.avatar_url)
                    .prop('class', 'sp-avatar sp-max-w-2xs'))
                .append('&nbsp; ')
                .append($('<span>').addClass('sp-description').text(value.user.formatted_name))
                .appendTo(ticketList);
        });
    ...
    var appendTicketsToList = function ($list, tickets, selectedTicket) {
        $.each(tickets, function (index, ticket) {
            var $option = $('<option/>')
                .val(ticket.id)
                .text("#" + ticket.number)
                .attr('data-data', JSON.stringify(ticket));
    ...
    var registerSelectize = function ($context, tickets) {
        var params = {
            ...
            load: function (query, callback) {
                ...
                var data = {term: query, exclude_ids: tickets};
                if (! query.length) {
                    data = {include_ids: recentTicketIds, exclude_ids: tickets}
                }

                $.get(laroute.route('ticket.operator.action.search'), data)
                    .done(function (res) {
                        callback(res.data);
                    })
                    .fail(function () {
    ...
    TicketAction.merge = function (tickets) {
        ...
        var makeInterface = function (ticketInfo) {
            ...
            return $('<div />')
                .append(createList(selectedTickets))
                .append('<br />')
                .append(
                    $('<form />')
                        .append($('<label />').text(Lang.get('ticket.merge_tickets_into')))
                        .append($select)
                )
                .html();
        };
        ...
        return Swal.fire({
            ...
            preConfirm: function () {
                ...
                return $.ajax({
                    ...
                    type: 'POST',
                    data: { 'ticket': tickets, 'merge_into_id': $('#swal2-select').val() },
                    dataType: 'json'
                }).then(function (response) {
                    if (response.status == 'success') {
                        return response;
    ...
    TicketAction.link = function (model) {
        ...
        var makeInterface = function (ticketInfo) {
            var $select = $('<select/>')
                .attr('name', 'merge_into_id')
                .attr('id', 'swal2-select')
                .append($('<option />'));
