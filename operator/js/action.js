$(function () {
    ...
    var route = $(this).data('route'),
      ...
      getCodeMirror = function ($element) {
        if ($element[0].sourcecode) {
          return $element[0].sourcecode.getContent();
        }
        return '';
      },
      data = {
  ...
  function initAction($dropdown) {
    ...
    $action.find('textarea.text').each(function () {
      ...
      // Parameters for mentions, use brand and department ID only for follow ups.
      if (typeof ticket !== 'undefined') {
        opts['brandId'] = ticket.parameters().brandId;
        opts['departmentId'] = ticket.parameters().departmentId;
      } else {
        opts['groupIds'] = groupIds;
      }
      ...
      $(this).selectize($.extend({}, emailSelectizeConfig(['restore_on_backspace', 'remove_button']), {
        ...
        load: function (query, callback) {
          ...
          $.get(laroute.route('user.operator.search'), {
            brand_id: typeof ticket !== 'undefined' ? ticket.parameters().brandId : null,
            q: query
          }).done(function (res) {
