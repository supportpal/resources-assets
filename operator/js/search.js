  ...
  var $selectize = $('.search-form .search').selectize({
    ...
    render: {
      optgroup_header: function (item, escape) {
        return '<div class="sp-search-category sp-p-3 sp-pb-2 sp-text-secondary sp-uppercase">' + '<strong>' + escape(item.label) + '</strong>' + +'</div>';
      },
      option: function (item, escape) {
        if (item.id !== 0 && item.id !== '') {
          return "<div>" + "<a class='sp-block sp-px-3 sp-py-1 sp-truncate sp-leading-snug' href='" + escape(item.link) + "'>" + "<span class='result-id sp-tag sp-bg-primary-800 sp-text-white'>" + escape("#" + item.id.toString()) + "</span>" + "<span class='result-name'>" + item.label + "</span><br />" + "<span class='result-secondary sp-description'>" + item.secondary + "</span>" + "</a>" + "</div>";
        } else if (item.id === '') {
          return "<div class='sp-mt-1'>" + "<a class='sp-block sp-p-3 sp-bg-secondary sp-text-sm sp-font-semibold' href='" + escape(item.link) + "'>" + item.label + "</a>" + "</div>";
        } else {
    ...
    load: function (query, callback) {
      ...
      $.post(laroute.route('core.operator.search_preview'), {
        ...
        self.addOptionGroup(Lang.choice('user.user', 2), {
          label: Lang.choice('user.user', 2)
        });
        self.addOptionGroup(Lang.choice('ticket.ticket', 2), {
          label: Lang.choice('ticket.ticket', 2)
        });
        self.addOptionGroup(Lang.choice('user.organisation', 2), {
